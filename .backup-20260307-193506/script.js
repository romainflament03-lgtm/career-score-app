...
function drawRadarChart(dimensions) {
  ...
  const labelFontSize = useMobileTuning ? 10 : (isMobileRadar ? 10 : 12);
  const labelLineHeight = useMobileTuning ? 11 : (isMobileRadar ? 11 : 13);
  ...
  const maxMobileLabelWidth = 68;

  ctx.font = `600 ${labelFontSize}px Manrope, sans-serif`;
  const splitLabelLines = (label) => {
    if (useMobileTuning) return [label]; // Mobile must keep labels on a single line.
    if (!isMobileRadar || ctx.measureText(label).width <= maxMobileLabelWidth) return [label];
    const middle = Math.floor(label.length / 2);
    let bestSplit = middle;
    let bestWidth = Number.POSITIVE_INFINITY;
    for (let i = Math.max(3, middle - 4); i <= Math.min(label.length - 3, middle + 4); i += 1) {
    ...
    return [label.slice(0, bestSplit), label.slice(bestSplit)];
  };
  const labelBlocks = labels.map((label) => {
    const lines = splitLabelLines(label);
    const width = Math.max(...lines.map((line) => ctx.measureText(line).width));
    const height = lines.length * labelLineHeight;
  ...
  const outerPadding = useMobileTuning ? 6 : (isMobileRadar ? 8 : 12);
  ...
  const maxLabelRing = Math.max(80, Math.min(maxLabelRingBySide, maxLabelRingByVertical));

  let polygonRadius = useMobileTuning
    ? Math.min(72, center - 66)
    : (isMobileRadar ? 112 : Math.min(132, center - 60));
  ...
  const minPolygonRadius = useMobileTuning ? 58 : (isMobileRadar ? 76 : 86);
  ...
  angles.forEach((angle, idx) => {
    ...
    const bubbleRadius = useMobileTuning
      ? Math.max(44, Math.min(bubbleRingRadius, pointRadius + 8))
      : Math.max(52, Math.min(bubbleRingRadius, pointRadius + 14));
  ...
  const sideVerticalOffset = useMobileTuning ? 20 : (labelFontSize + (isMobileRadar ? 8 : 6));
  labels.forEach((label, idx) => {
    ...
    if (useMobileTuning) {
      if (idx === 0) { // Top
        ...
        ctx.textBaseline = "middle";
        x = center;
        y = Math.min(y - 2, bubblePoint.y - bubbleHalfY - labelToBubbleGap - labelHalfH - 2);
      } else if (idx === 1) { // Right
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        x = Math.max(x + 2, bubblePoint.x + bubbleHalfX + labelToBubbleGap + (labelHalfW * 0.2));
        y = center - sideVerticalOffset;
      } else if (idx === 2) { // Bottom
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        x = center;
        y = Math.max(y + 2, bubblePoint.y + bubbleHalfY + labelToBubbleGap + labelHalfH + 2);
      } else { // Left
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        x = Math.min(x - 2, bubblePoint.x - bubbleHalfX - labelToBubbleGap - (labelHalfW * 0.2));
        y = center + sideVerticalOffset;
      }
    } else if (idx === 0) { // Top
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      y = Math.min(y - 2, bubblePoint.y - bubbleHalfY - labelToBubbleGap - (labelFontSize * 0.55));
    } else if (idx === 1) { // Right
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      x = x + 6;
      y = y - sideVerticalOffset;
    } else if (idx === 2) { // Bottom
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      y = Math.max(y + 2, bubblePoint.y + bubbleHalfY + labelToBubbleGap + (labelFontSize * 0.55));
    } else { // Left
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      x = x - 6;
    ...
    // Robust anti-collision: if label area intersects expanded bubble area, push label outward.
    ...
    const pushStep = useMobileTuning ? 5 : 4;
    const maxPush = useMobileTuning ? 10 : 6;
    for (let n = 0; n < maxPush; n += 1) {
      ...
      if (idx === 0) y -= pushStep;
      else if (idx === 1) x += pushStep;
      else if (idx === 2) y += pushStep;
      else x -= pushStep;
    }
    ...
    if (useMobileTuning) {
      ...
      const labelTop = y - labelHalfH;
      const labelBottom = y + labelHalfH;
      const stillIntersects = !(labelRight < bubbleLeft || labelLeft > bubbleRight || labelBottom < bubbleTop || labelTop > bubbleBottom);
      if (stillIntersects) {
        if (idx === 1) y = Math.max(labelHalfH + outerPadding, bubbleTop - labelHalfH - 3);
        if (idx === 3) y = Math.min(size - labelHalfH - outerPadding, bubbleBottom + labelHalfH + 3);
      }
    }
    ...
    block.lines.forEach((line, lineIndex) => {
      const lineY = y - (((block.lines.length - 1) * labelLineHeight) / 2) + (lineIndex * labelLineHeight);
